"""
Same comparison as topic_classification.py (Bag-of-Words vs. TF-IDF+custom
"Enriched" features, across Naive Bayes / KNN / Random Forest), but using
data/features/topic_classification_features_per_topic.csv instead of
data/features/topic_classification_features.csv as the custom feature source.

topic_classification.py's enriched representation appends 4 custom columns
per post: the score against only the BEST-matching topic (document_custom_score
etc., after taking an argmax over topics). This script instead appends all
4 score types x 4 topics = 16 columns (one set per topic, never argmax'd), so
the classifier sees the full per-topic signal instead of a single collapsed
score. Kept as a separate script/output so the two representations can be
...
FEATURES_PATH = os.path.join(BASE_DIR, "data", "features", "topic_classification_features_per_topic.csv")
TEXT_DATASET_PATH = os.path.join(BASE_DIR, "data", "features", "hackernews_topic_features_dataset.csv")
TEXT_COLUMN = "text_no_stopwords"
TOPIC_PREFIXES = ["artificial_intelligence", "climate_change", "cryptocurrency", "cybersecurity"]
SCORE_SUFFIXES = ["custom_score", "unigram_score", "bigram_score", "positional_score"]
CUSTOM_FEATURE_COLUMNS = [
    f"{prefix}_{suffix}" for prefix in TOPIC_PREFIXES for suffix in SCORE_SUFFIXES
]
VECTORIZER_PARAMS = {
    "lowercase": True,
    "ngram_range": (1, 1),
...
COMPARISON_OUTPUT = os.path.join(TABLES_DIR, "topic_classification_per_topic_representation_comparison.csv")
FIGURE_OUTPUT = os.path.join(FIGURES_DIR, "topic_classification_per_topic_representation_comparison.png")
...
def evaluate_representation(classifier_name, model, X_train, X_test, y_train, y_test, representation_label):
    if classifier_name == "K-Nearest Neighbors":
        # See topic_classification.py: KNN is distance-based, so raw BoW counts
        # and the enriched matrix (whose custom score columns break TF-IDF's
        # L2 norm once hstacked on) need to be put back on a comparable scale.
...
def plot_comparison(comparison_df, output_path):
    ...
    fig.suptitle("Κατηγοριοποίηση Θεμάτων: Bag-Of-Words έναντι TF-IDF με Per-Topic Enriched Features", fontsize=12, y=1.04)
    fig.legend(
    ...
    for classifier_name, model in classifiers:
        ...
        results.append(
            evaluate_representation(
                ...
                "Enriched (TF-IDF + 16 per-topic custom)"
            )
        )
